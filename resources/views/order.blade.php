...
@section('content')
    ...
    <div class="info w-full" style="background-image: url('../image/sublogo.jpg')">
        ...
        <br>
        <br>
        <h2 class="text-white text-xl text-end mt-10 w-5/6 m-auto pb-10"><a href="{{route('main')}}">Home</a> » <a href="{{route('catalog')}}">Catalog</a> » Order</h2>
                    ...
                    @foreach($order_items as $index => $order_item)
                        <tr  class="border border-gray-200 px-4 py-2 hover:bg-black/20">
                            <td class="border border-gray-200 px-4 py-2 ">
                                <img src="data:image/png;base64,{{ base64_encode($order_item->product->image_url) }}" alt="Image" class="w-[50%] m-auto ">
                            </td>
                            <td class="border border-gray-200 px-4 py-2">
                                <a class="text-gray-200 hover:text-amber-600" href="{{ route('product', ['product' => $order_item->product->id]) }}">
                            ...
                            <td class="border border-gray-200 px-4 py-2">
                                <form class="inline-flex flex-row items-center justify-center">
                                    ...
                                    <input class="quantity-input mr-1 ml-1 text-gray-500 max-w-[30%] px-4 py-2 bg-black/40 border border-transparent focus:border-amber-600 focus:outline-none focus:ring-amber-200" type="text" value="{{ $order_item->count }}" data-product-id="{{ $order_item->product->id }}">
                            ...
                            <?php
                                $currentPrice = $order_item->product->price*$order_item->count;
                            ?>
                            $<span id="productTotal_{{$index}}" class="product-total">{{$currentPrice}}</span>
                                <input type="hidden" value="{{$order_item->product->price}}" class="price-product-value">
                            </td>
                            <td class="border border-gray-200 px-2 py-2">
                                <form class="ml-4" action="{{ route('removeFromWishlist') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="product_id" id="product_id" value="{{$order_item->product->id}}">
                ...
                <form action="{{route('changeOrder')}}" method="post">
                    ...
                    <div class="">
                        <div class="mb-4 flex flex-col">
                            <label for="select_delivery" class="block text-[20px] font-medium mb-2 basis-1/3 text-amber-600/70"><span class="text-amber-800 text-[24px]">*</span>Спосіб доставки</label>
                            <select type="" id="select_delivery" name="select_delivery" class="text-gray-500 w-4/6 px-4 py-2 border bg-black/40 border-transparent focus:border-amber-600 focus:outline-none focus:ring-amber-200">
                        ...
                        <div class="mb-4 flex flex-col">
                            <label for="select_payment" class="block text-[20px] font-medium mb-2 basis-1/3 text-amber-600/70"><span class="text-amber-800 text-[24px]">*</span>Спосіб оплати</label>
                            <select type="" id="select_payment" name="select_payment" class="text-gray-500 w-4/6 px-4 py-2 border bg-black/40 border-transparent focus:border-amber-600 focus:outline-none focus:ring-amber-200">
                    ...
                    <div class="flex flex-row justify-end">
                        ...
                        <button type="submit" class="w-1/4 bg-amber-600 text-white py-2 hover:bg-amber-500 text-center mt-10">Continue</button>
                    </div>
                </form>
    ...
    <script>
        ...
        const totalOrderPrice = document.querySelector('.totalCartPrice');
        const minValue = 1;
        ...
        function updateProductTotal(index, price, quantityChange = 0) {
            ...
            if (quantityInput && productPriceElement) {
                ...
                productPriceElements.forEach(function (element) {
                    totalOrder += parseFloat(element.textContent);
                });

                totalOrderPrice.textContent = totalOrder.toFixed(0);
            }
        }
